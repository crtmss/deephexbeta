...
export function resolveAttack(attacker, defender, weaponId) {
  ...
  __t('COMBAT_RESOLVE', { weaponId: w.id, dist, finalDamage });
  return {
    distance: dist,
    baseDamage,
    armorClassMult,
...
export function validateAttack(attacker, defender, weaponId) {
  ...
  if (dist < rmin) { __t('COMBAT_VALIDATE', { ok:false, reason:'too_close', dist, rmin, rmax, weaponId }); return { ok: false, reason: 'too_close', distance: dist }; }
  if (dist > rmax) { __t('COMBAT_VALIDATE', { ok:false, reason:'out_of_range', dist, rmin, rmax, weaponId }); return { ok: false, reason: 'out_of_range', distance: dist }; }

  __t('COMBAT_VALIDATE', { ok:true, dist, rmin, rmax, weaponId });
  return { ok: true, distance: dist };
}  } catch (e) {
    __t('COMBAT_RESOLVE_EX', { weaponId, msg: String(e?.message || e) });
    return { distance: NaN, finalDamage: 0, weaponId };
  }
// src/units/CombatResolver.js
//
// Pure combat math. No Phaser imports.
