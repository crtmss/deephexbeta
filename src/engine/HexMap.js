  ...
  if (biome === 'icy') {
    ...
    if (seed) {
      ...
      cluster.forEach(t => {
        ...
        t.impassable = false;
        if (typeof t.elevation !== 'number') t.elevation = 1;
      });
    ...
    const mountains = flat.filter(t =>
      t.type === 'mountain' || (typeof t.elevation === 'number' && t.elevation === 4)
    );
    let hub = null;
    if (mountains.length) {
    ...
    if (seed) {
      ...
      for (const t of core) {
        for (const [x, y] of neighbors(t.q, t.r, map)) {
          ...
          if (!core.includes(nt)) {
            nt.type = 'grassland';
            if (typeof nt.elevation !== 'number' || nt.elevation > 1) {
              nt.elevation = 1;
            }
          }
          ringSet.add(nk);
...
function generateMap(rows = 25, cols = 25, seedStr = 'defaultseed', rand) {
  ...
  // Objects / POIs (forests/ruins/vehicles/roads) — NOW SEEDED
  const flat = map.flat();
