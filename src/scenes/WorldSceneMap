...
export function setupMap(scene) {
    scene.hexMap = new scene.HexMap(scene.mapWidth, scene.mapHeight, scene.seed);
    scene.mapData = scene.hexMap.getMap();
    scene.tileMap = {};
    ...
    scene.mapData.forEach(hex => {
        ...
        drawHex(scene, q, r, x, y, scene.hexSize, color);
    });
}
...
export function drawHex(scene, q, r, x, y, size, color) {
