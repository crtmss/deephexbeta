...
// - Roads lore is generated as a deterministic list (scene.loreState.roadEvents)
...
//
// FIX (2025-12-24):
// - Removed recursion/loop: ensureWorldLoreGenerated -> generateRoadLore -> ensureWorldLoreGenerated
// - Added scene.__generatingWorldLore guard.
...
/**
 * INTERNAL:
 * Generates the whole lore once. Safe to call multiple times.
 */
function ensureWorldLoreGenerated(scene) {
  if (!scene) return;
  if (scene.__worldLoreGenerated) return;

  // Guard against accidental recursion
  if (scene.__generatingWorldLore) return;
  scene.__generatingWorldLore = true;
  ...
  try {
    ...
    const anyLand = anyLandRaw.filter((t) => t && !isMountainTile(t));
    ...
    const isCoast = (t) => {
      ...
      if (isMountainTile(t)) return false;
    ...
    // IMPORTANT: create loreState skeleton EARLY so road lore can read context safely
    scene.loreState = scene.loreState || {};
    scene.loreState.islandName = islandName;
    scene.loreState.factions = factions;
    scene.loreState.resources = resInfo;
    ...
    const mineCount = highLand.length ? Math.floor(rng() * 3) : 0;
    ...
    const towerCount = highLand.length ? Math.floor(rng() * 3) : 0;
    ...
    const shrineCount = (forestLand.length || inlandLand.length) ? Math.floor(rng() * 3) : 0;
    ...
    const roadsideCount = 1 + Math.floor(rng() * 2);
    ...
    const raiderCampCount = (rng() < 0.65) ? Math.floor(rng() * 3) : 0;
    ...
    const surveyCount = Math.min(3, Math.max(2, 2 + Math.floor(rng() * 2)));
    ...
    scene.loreState.roadStartYear = roadStartYear;
    scene.loreState.__roadYearCursor = roadStartYear;
    ...
    // IMPORTANT:
    // We DO NOT force road events here if roadConnections aren't ready yet.
    // We can still build a deterministic list later when roadConnections exist.
    // But calling generator is safe; it will not recurse and will return empty if no connections.
    generateRoadLoreForExistingConnections(scene);
    ...
    const namedPlaces = [].concat(settlements).concat(ruins);
    ...
    scene.loreState = {
      ...
      roadEvents: Array.isArray(scene.loreState?.roadEvents) ? scene.loreState.roadEvents : [],
    };
    ...
    scene.__generatingWorldLore = false;
  }
}
...
/**
 * Roads lore builder:
 * - Does NOT call ensureWorldLoreGenerated (prevents recursion).
 * - If roadConnections are not ready yet, it just produces [].
 * - Stores deterministic list in scene.loreState.roadEvents.
 */
export function generateRoadLoreForExistingConnections(scene) {
  ...
  // Prepare loreState container (do not assume world lore already ran)
  scene.loreState = scene.loreState || {};
  scene.loreState.roadEvents = scene.loreState.roadEvents || [];

  if (!conns.length) {
    scene.loreState.roadEvents = [];
  ...
  const factions = Array.isArray(scene.loreState?.factions) ? scene.loreState.factions : [];
...
function buildLocationLabel(endpoint, tile, isFrom) {
  ...
  if (type === "crash_site") return `a crash site at (${q},${r})`;
  if (type === "wreck") return `a shipwreck near (${q},${r})`;
  if (type === "vehicle" || type === "abandoned_vehicle") return `a stranded vehicle at (${q},${r})`;
  if (type === "raider_camp") return `a raider camp at (${q},${r})`;
  if (type === "roadside_camp") return `a roadside camp at (${q},${r})`;
  if (type === "watchtower") return `a watchtower at (${q},${r})`;
  if (type === "mine") return `a mine at (${q},${r})`;
  if (type === "shrine") return `a shrine at (${q},${r})`;
  if (type === "ruin") return `ruins at (${q},${r})`;
  if (type === "settlement") return `a settlement at (${q},${r})`;
