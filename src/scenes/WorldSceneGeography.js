...
//   - Detect simple coastline tiles (no longer drawn visually).
//   - Read geoLandmark from HexMap.worldMeta and draw emoji + label.
//   - Optional overlay drawing (currently disabled for coastline rings).
...
export function resolveBiome(scene, map) {
  ...
  if (scene?.hexMap?.worldMeta?.biome) {
...
export function outlineColorFor(biome) {
  ...
  if (b.includes('icy'))     return 0x1e88e5;
  if (b.includes('volcan'))  return 0xd32f2f;
  if (b.includes('desert'))  return 0xfdd835;
  if (b.includes('swamp'))   return 0x4e342e;
  return 0x43a047;
}
...
export function initOrUpdateGeography(scene, map) {
  ...
  for (const t of map) {
    ...
    for (const [nq, nr] of neighborsOddR(q, r)) {
 ...
 *
 * Previously drew coastline circles — REMOVED.
 * Now:
 *   - Clears overlay layer
 *   - Draws ONLY landmark emoji + label
 * ========================================== */
export function drawGeographyOverlay(scene) {
  ...
  // -----------------------------------------
  // REMOVE all coastline outline drawing
  // -----------------------------------------
  g.clear();  // overlay is now blank unless we draw a landmark

  // -----------------------------------------
  // Landmark (emoji + label)
  // -----------------------------------------
  ...
  if (!scene.__geoLandmarkIcon || !scene.__geoLandmarkLabel) {
    ...
    const label = lm.label || resolveBiome(scene, map);
    ...
    if (scene.locationsLayer) scene.locationsLayer.add(txt);

    Object.defineProperty(scene, '__geoLandmarkIcon', { value: icon, enumerable: false });
    Object.defineProperty(scene, '__geoLandmarkLabel', { value: txt, enumerable: false });
  }
}
